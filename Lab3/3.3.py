...
for i in range(start_frame + 1, end_frame, step):
    ...
    if BGN_median is None:
        BGN_median = curr_gray.astype(np.float64)  # Convert to float64
    else:
        BGN_median = np.where(curr_gray < BGN_median, BGN_median + 1,
                            np.where(curr_gray > BGN_median, BGN_median - 1, BGN_median))
    ...
    if prev_mask is not None:
        # Update background model using pixels classified as background in the current object mask
        BUF_mean[prev_mask == 0] = alpha * curr_gray[prev_mask == 0] + (1 - alpha) * BUF_mean[prev_mask == 0]
        BGN_median[prev_mask == 0] += np.where(curr_gray[prev_mask == 0] < BGN_median[prev_mask == 0], 1,
                                               np.where(curr_gray[prev_mask == 0] > BGN_median[prev_mask == 0], -1, 0))

    # Update previous object mask
    prev_mask = binarized_image_median.copy()
...
# Print evaluation metrics for median approximation
